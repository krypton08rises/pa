...
class VadChunkStream:
    ...
    def read_utterance(self) -> np.ndarray | None:
        ...
        while True:
            try:
                if (_chunk := self.utterance_queue.get(timeout=0.5)) is not None:
                    return _chunk
    ...
    def _flush_utterance(self, reseed_pre_roll: bool = False) -> None:
        """
        Concatenate and enqueue the current utterance buffer, retrying on a
        full queue until it succeeds or stop_event is set.
        """
        payload = np.concatenate(self.utterance_buffer, axis=0)
        while True:
            try:
                self.utterance_queue.put(
                    payload, timeout=self.config.thread_timeout_seconds
                )
                break
            except queue.Full:
                if self.stop_event.is_set():
                    logger.warning(
                        "Utterance queue full and stop_event set - dropping utterance."
                    )
                    break
                logger.warning("Utterance queue full, retrying put...")
        ...
        with AudioStream(self.recording_config) as audio_stream, torch.no_grad():
            while True:
                ...
                if self.max_utterance_seconds_reached():
                    ...
                    self._flush_utterance(reseed_pre_roll=True)
